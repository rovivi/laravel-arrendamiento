		...
		<div class="form-group backpack-filter m-b-0">
			<div class="input-group date">
		        ...
		        <input class="form-control pull-right"
		        		id="datepicker-<?php echo e(str_slug($filter->name)); ?>"
		        		type="text"
						<?php if($filter->currentValue): ?>
							value="<?php echo e($filter->currentValue); ?>"
						<?php endif; ?>
		        		>
		        <div class="input-group-addon datepicker-<?php echo e(str_slug($filter->name)); ?>-clear-button">
...
<?php $__env->startPush('crud_list_styles'); ?>
    <link rel="stylesheet" href="<?php echo e(asset('vendor/adminlte/bower_components/bootstrap-datepicker/dist/css/bootstrap-datepicker3.css')); ?>">
	<style>
		.input-group.date {
			width: 320px;
			max-width: 100%;
...
<?php $__env->startPush('crud_list_scripts'); ?>
  ...
  <script>
		jQuery(document).ready(function($) {
			var dateInput = $('#datepicker-<?php echo e(str_slug($filter->name)); ?>').datepicker({
				autoclose: true,
				format: 'yyyy-mm-dd',
				todayHighlight: true
			})
			...
			$('li[filter-name=<?php echo e(str_slug($filter->name)); ?>]').on('filter:clear', function(e) {
				...
				$('#datepicker-<?php echo e(str_slug($filter->name)); ?>').datepicker('update', '');
				$('#datepicker-<?php echo e(str_slug($filter->name)); ?>').trigger('changeDate');
			});

			// datepicker clear button
			$(".datepicker-<?php echo e(str_slug($filter->name)); ?>-clear-button").click(function(e) {
				e.preventDefault();
